...
from .simulator_wrapper import OBS_Processing
...
class Simulator(gym.Env):
    ...
    def __init__(
        ...
        if self.randomize_maps_on_reset:
            ...
            self.map_names = [
                _map for _map in self.map_names if not _map.startswith(("calibration", "regress")) or not _map.startswith(("kmap"))
            ]
    ...
    def reset(self, segment: bool = False):
        ...
        obs = OBS_Processing.render_obs(self,segment=segment)
        #obs = render_obs(segment=segment)
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        reward = self.reward_distance_travelled() + self.reward_posangle() + self.reward_velocity() + self.reward_colision() + self.reward_proximity_penalty()
        return reward *10
    ...
    def step(self, action: np.ndarray):
        ...
        #obs = self.render_obs()
        obs = OBS_Processing.render_obs(self,segment=False)
    ...
    #TODO: start reward measurementssby Kalapos
    def reward_distance_travelled(self): 
    ...
    def reward_proximity_penalty(self):
        proximity_penalty = self.unwrapped.proximity_penalty2(self.unwrapped.cur_pos, self.unwrapped.cur_angle)
        self.proximity_reward = proximity_penalty * 2.5
        #logger.debug("Proximity reward: {:.3f}".format(self.proximity_reward))
        return self.proximity_reward
    
     #TODO: end reward Kalapos

    def _compute_done_reward(self) -> DoneRewardInfo:
        # If the agent is not in a valid pose (on drivable tiles)
        if not self._valid_pose(self.cur_pos, self.cur_angle,2.0):
